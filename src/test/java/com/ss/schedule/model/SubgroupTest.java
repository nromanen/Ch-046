...
public class SubgroupTest {
    ...
    public void testCanBeAddedToTimetableManager() throws Exception {
        ...
        Subgroup subgroup=FCS.getGroups().get(0).getSubgroups().get(0);
        boolean canBeAddedToTimetableManager = subgroup.canBeAddedToTimetableManager(timeTableManager);
        assertEquals(canBeAddedToTimetableManager,false);
    }

    @Test
    public void testIsPresentOrEquals() throws Exception {
        Subgroup subgroup=FCS.getGroups().get(0).getSubgroups().get(0);
        assertTrue(subgroup.isPresentOrEquals(FCS.getGroups().get(0)));
    }
