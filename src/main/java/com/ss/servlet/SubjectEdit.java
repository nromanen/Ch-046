...
public class SubjectEdit extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * TeachersSubjectsDao tsd = new TeachersSubjectsDao(); TeacherDao td =
		 * new TeacherDao(); int teacherId =
		 * Integer.parseInt(request.getParameter("teacherId")); int subjectId =
		 * Integer.parseInt(request.getParameter("subjectId")); String
		 * teacherName = td.getById(teacherId).toString(); tsd.delete(subjectId,
		 * teacherId); List<Subject> list = tsd.getSubjects(teacherId);
		 * request.setAttribute("name",teacherName);
		 * request.setAttribute("subjects", list);
		 * request.getRequestDispatcher("/WEB-INF/view/subjects.jsp").forward(
		 * request, response);
		 */
