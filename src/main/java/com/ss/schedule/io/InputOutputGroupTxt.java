...
public class InputOutputGroupTxt implements InputOutput<List<Group>> {
	...
	@Override
	public List<Group> readFromFile(String filePath) {
	...
	@Override
	public void writeToFile(String filePath, List<Group> groups) {
		if (groups.size() == 0) {
		...
		writeStringsToFile(new File(filePath), formattedGroups);
	}
